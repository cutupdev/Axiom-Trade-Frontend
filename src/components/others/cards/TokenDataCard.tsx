...
export function TokenTableHeader() {
  return (
    <div className="sticky top-0 z-20 flex flex-row items-center px-2 sm:px-[13px] border-b border-[#333333] w-full min-w-fit h-[40px] sm:h-[48px] lg:h-[53px] bg-[#0D1117] text-[#A0A0A0] text-[10px] sm:text-[11px] lg:text-[12px] font-medium">
      ...
      <div className="flex justify-start items-center gap-1 px-3 sm:px-2 lg:px-3 w-[190px] sm:w-[160px] lg:w-[180px] flex-shrink-0">
        Token Info
      </div>
...
export default function TokenDataCard({
  item,
  index,
  hiddenIds,
  setHiddenIds,
  ...
  const formatSince = (creationTimestamp?: number) => {
    if (!creationTimestamp || !Number.isFinite(Number(creationTimestamp)))
      return "-";
    ...
    <div className="group flex flex-row items-center bg-[#0A0D12] hover:bg-[#161B22] active:bg-[#22c55e]/10 px-2 sm:px-[13px] border-[#21262D] border-b-[1px] w-full min-w-fit h-[90px] sm:h-[100px] lg:h-[110px] cursor-pointer">
      ...
      <div className="flex flex-col justify-center items-start gap-0.5 sm:gap-1 px-3 sm:px-2 lg:px-3 w-[130px] sm:w-[120px] lg:w-[140px] flex-shrink-0">
        <span className="font-medium text-[11px] sm:text-[13px] lg:text-[14px] text-white truncate">
          ${item.marketCapUSD ? (Number(item.marketCapUSD) / 1000).toFixed(1) : 0}K
        </span>
        <span
        ...
        >
          {Number(item.priceChange5mPerc) >= 0 ? '+' : ''}{(Number.isFinite(Number(item.priceChange5mPerc)) ? Number(item.priceChange5mPerc).toFixed(2) : 0)}%
        </span>
      </div>
      ...
      {/* Token Info Column - Fixed width */}
      <div className="flex flex-col gap-0.5 sm:gap-1 px-3 sm:px-2 lg:px-3 w-[190px] sm:w-[160px] lg:w-[180px] flex-shrink-0">
        {/* Row 1 */}
        <div className="flex flex-row gap-1">
          <div className="flex items-center gap-1 text-[11px]">
            <span className={Number(item.top10HoldersSupplyPerc || 0) <= 20 ? "text-green-500" : "text-red-500"}>
              ⬆ {Math.abs(Number(item.top10HoldersSupplyPerc || 0)).toFixed(2)}%
            </span>
          </div>
          <div className="flex items-center gap-1 text-[11px]">
            <span className={Number(item.devHoldingSupplyPerc || 0) <= 5 ? "text-green-500" : "text-red-500"}>
              ⬇ {Math.abs(Number(item.devHoldingSupplyPerc || 0)).toFixed(2)}%
            </span>
          </div>
          <div className="flex items-center gap-1 text-[11px]">
          ...
          <div className="flex items-center gap-1 text-[11px]">
            <CiUser className="text-[12px]" />
            <span className={`${(item.holders ?? 0) >= 100 ? "text-green-500" : "text-red-500"}`}>
              {item.holders ?? 0}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-row gap-1">
          <div className="flex items-center gap-1 text-[11px]">
            <span className="text-green-500">⬆ {Number(item.insiderWalletsSupplyPerc || 0).toFixed(2)}%</span>
          </div>
          <div className="flex items-center gap-1 text-[11px]">
            <span className="text-red-500">⬇ {Math.abs(Number(item.priceChange5mPerc || 0)).toFixed(2)}%</span>
          </div>
          <div className="flex items-center gap-1 text-[11px]">
            <span className={`${Number(item.bondingCurveProgress || 0) >= 50 ? "text-green-500" : "text-red-500"}`}>
              ⬆ {Number(item.bondingCurveProgress || 0).toFixed(1)}%
            </span>
          </div>
          <div className="flex items-center gap-1 text-[11px]">
            <TbChartCandle className="text-[12px]" />
            <span className={`${(Array.isArray(item.chart?.c) ? item.chart.c.length : 0) >= 10 ? "text-green-500" : "text-red-500"}`}>
              {Array.isArray(item.chart?.c) ? item.chart.c.length : 0}
            </span>
          </div>
        </div>
        ...
        {/* Row 3 - Status badges */}
        <div className="flex flex-row gap-1">
          <div className="flex items-center gap-1 bg-green-500/20 px-2 py-0.5 rounded text-[10px] text-green-500">
            ⬆ Paid
          </div>
          <div className="flex items-center gap-1 bg-red-500/20 px-2 py-0.5 rounded text-[10px] text-red-500">
            ⬇ Unpaid
          </div>
        </div>
      </div>
